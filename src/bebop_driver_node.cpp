...
BebopDriverNode::BebopDriverNode()
  ...
  {
    auto param_desc = rcl_interfaces::msg::ParameterDescriptor{};
    param_desc.description = "The frame id of the odometry frame of reference";
  ...
  // animationFlip
  subscription_animationFlip = this->create_subscription<std_msgs::msg::UInt8>(
      "flip", 1, [this](const std_msgs::msg::UInt8::SharedPtr msg) -> void {
  ...
  subscription_cmdVel = this->create_subscription<geometry_msgs::msg::Twist>(
      "cmd_vel", 1,
      std::bind(&BebopDriverNode::cmdVelCallback, this, std::placeholders::_1));

  // moveCamera
  subscription_moveCamera =
  ...
  subscription_photo = this->create_subscription<std_msgs::msg::Bool>(
      "photo", 1, [this](const std_msgs::msg::Bool::SharedPtr msg) -> void {
  ...
  altitude_timer = this->create_wall_timer(
      66ms, std::bind(&BebopDriverNode::publishAltitude, this));
...
void BebopDriverNode::publishImu(void) {
  ...
  RCLCPP_DEBUG(this->get_logger(),
               "IMU: roll=%f, pitch=%f, yaw=%f, vel=(%f, %f, %f)", roll, pitch,
               yaw, vx, vy, vz);
  ...
  // Angular velocity (estimated from acceleration, not directly available from
  // Bebop) For now, we set to zero as Bebop doesn't provide raw angular
  // velocity
